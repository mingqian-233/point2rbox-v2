...
class Point2RBoxV2Head(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 voronoi_thres: dict = dict(
                     default=[0.994, 0.005],
                     override=(([2, 11], [0.999, 0.6]),
                               ([7, 8, 10, 14], [0.95, 0.005]))),
                 square_cls: list = [1, 9, 11],
                 edge_loss_cls: list = [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 13],
        ...
        self.loss_bbox_syn = MODELS.build(loss_bbox_syn)
    ...
    def interpolate_layers(self, x):
        HW = x[0].shape[-2:]
        y = [x[0]]
        for xi in x[1:]:
            y.append(nn.functional.interpolate(xi, HW, mode='nearest'))
    ...
    def forward(
        ...
        """
        V = self.interpolate_layers(x)
        K = self.conv_gate(V.view(-1, *V.shape[2:])).view(*V.shape[:2], *V.shape[3:]).sigmoid()

        self.coef_sin = self.coef_sin.to(K)
        self.coef_cos = self.coef_cos.to(K)
        phase_sin = torch.sum(K * self.coef_sin[3:9, None, None], dim=1, keepdim=True)
        phase_cos = torch.sum(K * self.coef_cos[3:9, None, None], dim=1, keepdim=True)
        phase = -torch.atan2(phase_sin, -phase_cos) / torch.pi + 1  # In range [0, 2)
        scale_pred = torch.pow(2, 6 * phase - 3)  # Map to [1, 32)

        gate_score = torch.argmax(K, dim=1, keepdim=True)
        KV = V.gather(1, gate_score[:, :, None].expand(-1, -1, *V.shape[2:]))[:, 0]
        cls_feat = KV
        reg_feat = KV
        ...
        bbox_pred = torch.stack((sig_x, sig_y, dx, dy), 1) * scale_pred * 8
        
        return (cls_score,), (bbox_pred,), (angle_pred,), (gate_score,)
    ...
    def loss_by_feat(
        ...
        if len(pos_inds) > 0:
            ...
            if torch.any(pos_syn_mask):
                ...
                loss_bbox_syn = gwd_loss((None, preds.bmm(preds)), targets)
            else:
            ...
            loss_bbox = self.loss_bbox(
                ...
                avg_factor=num_pos)
            loss_bbox += self.loss_bbox(
                pos_rbox_preds[~pos_syn_mask, 2:4], 
                pos_rbox_preds[~pos_syn_mask, 2:4].detach() - 1, 
                avg_factor=num_pos)
            ...
            if torch.any(valid):
                ...
                if ss_info[0] == 'rot':
                    ...
                    loss_ss = 2 * gwd_loss((None, ori_preds.bmm(ori_preds)), (None, trs_preds.bmm(trs_preds)))
                    d_ang = trs_angle - ori_angle - ss_info[1]
                    d_ang = (d_ang + math.pi / 2) % math.pi - math.pi / 2
                    d_ang[square_mask] = 0
                    loss_ssa = 2 * self.loss_ss(d_ang, torch.zeros_like(d_ang))
                elif ss_info[0] == 'flp':
                    ...
                    d_ang = (d_ang + math.pi / 2) % math.pi - math.pi / 2
                    d_ang[square_mask] = 0
                    loss_ssa = 0.5 * self.loss_ss(d_ang, torch.zeros_like(d_ang))
                else:
                    ...
                    d_ang = (d_ang + math.pi / 2) % math.pi - math.pi / 2
                    d_ang[square_mask] = 0
                    loss_ssa = 1.0 * self.loss_ss(d_ang, torch.zeros_like(d_ang))
            else:
    ...
    def predict_by_feat(
        ...
        for img_id in range(len(batch_data_samples[2])):
            ...
            gate_score_list = select_single_mlvl(
                gate_scores, img_id, detach=True)
            
            if hasattr(data_sample[0], 'bboxes'):
            ...
            results = predict_by_feat(
                ...
                gate_score_list=gate_score_list,
    ...
    def _predict_by_feat_single_pseudo(self,
        ...
        cls_score, bbox_pred, angle_pred, gate_score = cls_score_list[0], bbox_pred_list[0], angle_pred_list[0], gate_score_list[0]
        ...
        gate_score = gate_score.permute(1, 2, 0).reshape(-1, gate_score.shape[0])[gt_idx]
        ...
        # results.informs = torch.cat([cls_score.max(1)[1][:, None], gate_score], -1).float().round(decimals=2)
    ...
    def _predict_by_feat_single(self,
        ...
        results = InstanceData()
        results.bboxes = RotatedBoxes(bbox_pred)
