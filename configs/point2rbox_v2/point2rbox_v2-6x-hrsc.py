...
model = dict(
    ...
    bbox_head=dict(
        ...
        feat_channels=128,
        strides=[8],
        edge_loss_start_epoch=48,
        joint_angle_start_epoch=24,
        voronoi_type='gaussian-orientation',
        voronoi_thres=dict(default=[0.99, 0.02]),
        square_cls=[],
        edge_loss_cls=[0],
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=1.0),
        loss_bbox=dict(type='mmdet.L1Loss', loss_weight=0.01),
        loss_ss=dict(
            type='mmdet.SmoothL1Loss', loss_weight=5.0, beta=0.1)),
